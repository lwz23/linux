...
static int tty0tty_open(struct tty_struct *tty, struct file *file)
{
	...
	if ((index % 2) == 0) {
		printk("enter index 2==0 br");
		if (tty0tty_table[index + 1] != NULL){
		printk("pass index+1 !null check");
...
printk("tty0tty_close is called");
#ifdef SCULL_DEBUG
	printk(KERN_DEBUG "%s - \n", __FUNCTION__);
#endif
	if (tty0tty)
				    ...
				    unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == TIOCMIWAIT) {
		...
		//struct wait_queue_entry wait = __WAITQUEUE_INITIALIZER(wait, current);
